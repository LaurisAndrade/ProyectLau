    ...
    <div data-role="page" id="pagina_uno">
      ...
      <div data-role="main">
        ...
        <table data-role="table" id="table-custom-2" data-mode="columntoggle" class="ui-body-d ui-shadow table-stripe ui-responsive" data-column-btn-theme="b" data-column-btn-text="Columns to display..." data-column-popup-theme="a">          <thead>
          ...
          <?php
            $numeracion=1;
            foreach ($recomendaciones->result() as $row){
           ?>
            <tr>
              <!--<th><?= $numeracion;?></th>-->
              <td><?= nl2br($row->rec_descripcion);  ?></td>
              <td>
                <?php $estado=$row->rec_estado; 
                  if($estado==1){
                    echo "Activo";
                  }
                  else{
              ...
              <td><img src="<?= base_url(); ?><?= $row->rec_foto;?>" style="height:120px; width:120px; background-color:#67EC69;" alt=""></td>
              <td><a href="<?= base_url(); ?>index.php/admin/editar_recomendacion/<?= $row->rec_codigo; ?>" data-icon="home" data-ajax="false">Editar</a></td>
            </tr>
          <?php
            $numeracion++;
            }
